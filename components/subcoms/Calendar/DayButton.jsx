...
function getStyles(props, context, state) {
  const {date, disabled, selected, style} = props;
  const {hover} = state;
  const {baseTheme, datePicker} = context.muiTheme;
  let labelColor = Object.keys(style).length !==0 && style.color ? style.color : baseTheme.palette.textColor;
  let buttonStateOpacity = 0;
  let buttonStateTransform = 'scale(0)';
  if (hover || selected) {
    labelColor =Object.keys(style).length !==0 && style.color ? style.color : datePicker.selectTextColor;
  ...
  return {
    ...
    buttonState: {
      backgroundColor: Object.keys(style).length !==0 && style.backgroundColor ? style.backgroundColor : datePicker.selectColor,
      borderRadius: '50%',
      height: 34,
      left: `calc(50% - 17px)`,
